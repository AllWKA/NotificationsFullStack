...
function createClient(event) {
  ...
  if (checkCreateClientFrom()) {
    ...
    var method = "POST", url = "http://192.168.1.125:3000/user";
    var newClient = {
...
function deleteClient(event) {
  event.preventDefault();
  var xhttp = new XMLHttpRequest(), method = "DELETE", url = "http://192.168.1.125:3000/user/"
...
function getTokensNotifications() {
  return new Promise((resolve, reject) => {
    var xhttp = new XMLHttpRequest(), method = "GET", url = "http://192.168.1.125:3000/tokenNotificationsFromUser/"
      + inputEmail.placeholder + "/" + appFromClient.innerHTML;
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
...
function fillApplicationFromClient() {
  var xhttp = new XMLHttpRequest(), method = "GET", url = "http://192.168.1.125:3000/appsFromAdmin/" + inputEmail.placeholder;
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      applications = JSON.parse(xhttp.responseText);
      if (applications != null) {
...
function updateClient(event) {
  ...
  if (inputEmail.value == "" || userNameInput.value == "" || inputPassword.value == "") { alert("fill all inputs please!"); }
    ...
    var method = "PUT"
    var url = "http://192.168.1.125:3000/user/" + inputEmail.placeholder + "/" + appFromClient.innerHTML;
    var updatedClient = {
...
function listAllUsers() {
  var xhttp = new XMLHttpRequest(), method = "GET", url = "http://192.168.1.125:3000/users";
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
